...
will depend on `@unify-verse/atlas-core`.
...
R: Recommend `@unify-verse/atlas-core` (npm-publishable scoped name)
until Tether allocates a `@tetherto/*` scope. Defer scope decision to
reviewer. `package.json.name` below uses `@unify-verse/atlas-core` as a
placeholder.
